...
в `_dsts_ru_backups`. В корень игры также устанавливается небольшой
`dinput8.dll`: он исправляет ошибку PC-версии, из-за которой введённые кириллицей
имена превращались в `à/ä/ï`. Уже сохранённые испорченные имена этот модуль не
переписывает.

Исправление проверено при русской системной кодовой странице Windows (CP1251).
При включённом экспериментальном режиме UTF-8 или другой кодовой странице
ANSI игра может потерять кириллицу ещё до обработки модулем.

Если в папке игры уже есть сторонний `dinput8.dll`, установщик безопасно
остановится до внесения изменений, чтобы не сломать другой мод.
