...
CSV_ROOT = ROOT / "csv"
LOG_PATH = ROOT / "logs" / "fix_junomon_gender_tail_pass_v049.log"
...
def apply_updates(root_name: str) -> int:
    path = CSV_ROOT / root_name / "message/d09.mbe/000_Sheet1.csv"
    if not path.exists():
        return 0
    raw = path.read_bytes()
    ...
    for row in reader:
        if unpack_text(row.get("string2 0", "")) != "f_d0907_0040_0010":
            continue
        old_value = row["string 2"]
        decoded = unpack_text(old_value)
        updated = decoded.replace("Я\nбыл частью", "Я\nбыла частью", 1)
        if updated == decoded:
            continue
